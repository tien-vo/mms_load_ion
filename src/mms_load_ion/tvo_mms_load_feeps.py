__all__ = ["tvo_mms_load_feeps"]

from .utils import sampling_period, move_avg
...
def tvo_mms_load_feeps(trange, species="ion", probe="1", drate="brst"):
    ...
    mms_load_feeps(trange=trange, probe=probe, data_rate=drate, datatype=dtype, time_clip=True, spdf=True)

    # Preallocate
    tmp_data = tp_get_data(
    ...
    for i, (head, eye) in enumerate(instruments):
        nflux_data = tp_get_data(f"{prefix}_{head}_intensity_sensorid_{eye}_clean_sun_removed", xarray=True)
        percentage_err_data = tp_get_data(f"{prefix}_{head}_percent_error_sensorid_{eye}", xarray=True)
        # Unpack & sanity check
    ...
    eflux_omni_avg = move_avg(eflux_omni, (window, 1), smooth=True)

    del_data()
    return dict(
        t=t.astype("f8"),
        f_omni_energy=energy.to(u.eV),
        f_omni=eflux_omni,
...
if __name__ == "__main__":
    ...
    ion_data = tvo_mms_load_feeps(trange, species="ion", probe=probe, drate=drate)
    t_ion, energy_ion = np.meshgrid(ion_data["t"].astype("datetime64[ns]"), ion_data["f_omni_energy"], indexing="ij")
    elc_data = tvo_mms_load_feeps(trange, species="elc", probe=probe, drate=drate)
